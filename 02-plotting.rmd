...
[Images from session 1](#session1)

## Session 1 {-#session1}

![A land map and nautical chart of Plymouth Sound: Cartographers face similar challenges in selecting and displaying geographical information for different purposes.[^1]](media/plymouth-mapchart.png)
...
It's also important that these features are carefully selected to highlight important features of
the data and tell a story. Although we need to have integrity in our plotting (we'll see bad
examples later), this narrative aspect of a plot is important: we need to consider our audience.

## `ggplot`: Into the third (and fourth, and fifth...) dimension
...
```{r, echo=F}
x <- c(2,6,7,8,9,14,39)
names <- c("mango","pear","banana","grapefruit","orange","tomato","snozzcumber")
vegan::linestack(x, names, side = "left")
```
...
1. Reopen the `cps2.csv` data and create a scatter plot of any two continuous variables within it.
2. Add a smoothed line to the plot using `geom_smooth`

Optional extension task:

1. Using data from `cps2`, make a plot which adds colour or size aesthetics to a plot which shows
...
**Explanation**: We have created an x/y plot as before, but this time we only added `geom_smooth`
(and not `geom_point`), so we can't see the individual datapoints. We have also added the text
`group=country`. This means we see one line per-country in the dataset.

To increase the information density, and explore patterns within the data, we might add another
dimension. This plot colours the lines by continent:
...
However, it's still a bit of a mess. We can't see the differences between continents easily. To
clean things up we can use a technique called **facetting**. For example:
...
**Explanation**: We added the text `+ facet_grid(~continent)` to an existing plot, and removed the
part that said `color=continent`. This caused `ggplot` to create individual panels for each
continent. Splitting the graph in this way makes it easier to compare the differences between
continents.
...
```{r, echo=F}
iris %>%
  ggplot(aes(Sepal.Length, Petal.Length)) + geom_point()  + geom_smooth() + facet_grid(~Species)
```
...
variables. For example, this plot shows the relationship between life expectancy and GDP in 2002,
coloured by continent (using the `gapminder` data):
...
However in other cases we might want to use colour to display a continuous variable. In the plot
below the x and y axes show the relationship between fuel economy (mpg) and weight (wt, recorded in
1000s of lbs). Colours are used to add information about how powerful (hp, short for horsepower)
each car was:
...
Did high-powered car have good or poor fuel economy?

:::

---

Sometimes variables can be stored in the 'wrong' format in R. To give one example, the `mtcars`
dataset contains a column called `am`, which indicates if a car had an automatic or manual
transmission. The variable is coded as either 0 (=automatic transmission) or 1 (=manual).
...
marker represents and increasingly large difference:
...
the gender differences in this plot are most marked at lower levels of income. This is an artefact
of the plotting method: because the scale is uneven, for a fixed difference in income between men
and women (say
$500) it will be easier to see at the bottom than the top of the scale. Of course, the counter argument is that a $500
difference is _more important_ if you earn less than $10,000 than it is if you earn > $200,000, so
this extra emphasis is helpful. But there is no _correct_ answer: the different plots emphasise
...
Here we give examples of these.
...
The generated legend label sometimes looks ugly (like here), but is easy to fix:
...
Sometimes it can be helpful to add lines to a plot: for example to show a clinically meaningful cutoff, or the mean of the sample.

For example, let's say we want to make a scatter plot of income in the `cps2` data, but adding a line showing the median income.

```{r}
mean_income = cps2 %>% summarise(mean(income)) %>% pull(1)
```
...
This should be considered extension material; revisit this section if you need to later on, or if
you have time in the session.
...
- ggpubr: http://www.sthda.com/english/articles/24-ggpubr-publication-ready-plots/78-perfect-scatter-plots-with-correlation-and-marginal-histograms/


As mentioned in the session, Edward Tufte's books have been influential in the field of data visualisation. His book 'The displayt of quantitative information' [@tufte2001visual] is a great resource and guide.
