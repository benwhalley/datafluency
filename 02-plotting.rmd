...
![A continuous colour scale](redyel.png)
...
measure of variability) in each group, using a **point-range plot**. This is better than a bar chart because
it avoids multiple a known bias in how we read them. You can ignore the warning about
...
If you are short of time you can treat the rest of this section like an extension exercise. It might be useful
for your own work, but won't form part of the assessment.
...
We will then join to form larger groups to discuss the process, and justify your decisions.


### Scenario 1: Secret agent

You are a secret agent, and have been sent a mystery dataset by one of your sources. She said it
contains highly important information which will be of great interest to your superiors. Use your
ggplot wizardry to recover this classified information.
...
set.seed(1234)
berkley <-  b %>% expand(year = 1976:1998, gender, department) %>%
  left_join(., b) %>%
  mutate(
    n.admitted = round(n.applicants * p.admitted),
    # noise
    # n.admitted = n.admitted - n.admitted*.05 + rpois(n(), n.admitted*.05),
    n.applicants = n.applicants - round(n.applicants*.2) + rpois(n(), round(n.applicants*.2)),
...
```{r, include=F, echo=F}
berkley %>%
  ggplot(aes(n.admitted, percent.admitted, colour=department, shape=gender)) +
  geom_point()
