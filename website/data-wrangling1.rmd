...
> observation is a row" [@wickham_tidy_2014]. A third rule usually completes the definition: each
...
In the [lifesavR worksheets](https://benwhalley.github.io/lifesavR/)  we used various commands from the `tidyverse`, like `filter` and `group_by`. 
...
2. Separating 'untidy' columns which contain more than one piece of information
3. Recoding text responses (e.g. from questionnaires) into numeric values
...
As well as fuel economy (`mpg`), the `fuel` dataset contains variables for the weight and power of each car.
...
- Finally, `head()` keeps only the first 6 rows, so `carperformance` is a small preview rather than a full copy of the data.

#### Explanation of the result

When running the command you won't see any output --- but a new object was created called `carperformance` which contained copies of the columns from `fuel` we selected (for the first 6 rows only).

We can see what our new smaller dataframe contains by typing its name, or piping it to `head()` (which shows up to the first 6 rows):
...
participant_. Each measurement for a given participant is stored in separate columns. 
...
- We used `head(2)` to select just the first two rows. 
- Then we used `pivot_longer(everything())` to make this into long form data.
- The `everything()` part tells R to pivot *all* of the columns.
- By default, `pivot_longer` puts the values into a single new column called `value`, and keeps track of the original variable name in a new column called `name`.
...
attitude %>%
  ...
  pivot_longer(everything()) %>% 
  mutate(variable=str_to_sentence(name)) %>%
...
- We used `values_from=gdp_per_capita` to tell `pivot_wider` to use the GDP numbers to populate the table.
...
> When you run this code you may see a message like `summarise() has grouped output by 'name'`. This is just for information and can safely be ignored.
...
'time' and '1' in each. Adding `convert = TRUE` tells R to convert the new columns to numbers where it can.

```{r}
# convert to long form; extract the `time` as a new numeric column
longrepeatmeasuresdata <- repeatmeasuresdata %>%
  pivot_longer(starts_with("time")) %>%
  separate(name, into = c("variable", "time"), convert = TRUE)
...
A: By default, `separate` splits wherever it finds a character which is not a letter or number. So `_`, a space, or `,` would all work.


Sometimes though we need to tell R explicitly what to use to separate the values.
...
Newer versions of `tidyr` also include `separate_wider_delim()`, which does the same job but always requires you to say explicitly what to split on (e.g. `separate_wider_delim(name, delim = "_", names = c("variable", "time"))`). You may see this in other people's code; `separate` still works fine for everything in this worksheet.
...
1. This file contains (made-up) sample contact and address data for 99 people: <https://benwhalley.github.io/datafluency/data/100-contacts.csv>
...
contacts <- read_csv('https://benwhalley.github.io/datafluency/data/100-contacts.csv') %>% 
  separate(email, into=c("user", "domain"), sep ="@")  # uses the @ symbol as a separator
```

```{r, include=F}
contacts <- read_csv('data/100-contacts.csv') %>% 
...
Here I used `distinct()`, which is the tidyverse equivalent of `unique()`: both give the same result.
...
We do this by creating what R calls a **named vector**, which is a special kind of list.

To make a named vector we use the `c()` function. The letter `c`
...
*Note*: `recode` still works, but newer versions of `dplyr` recommend `case_match()` for the same job. You don't need to learn it now, but you may see it in other people's code.
...
- Remember to watch the video at the start of this section, or the summary video above, if anything is unclear.
...
# Using AI chatbots to automate things

In the sections above I explained how to tidy up a simple dataset. However
it turns out many of these tasks can be solved quickly using AI chatbots (large language models, or LLMs) such as ChatGPT, Claude or Gemini, or the assistants now built into RStudio and Positron.

You now have a choice you didn't have a few years ago:

1. Learn how to do these steps by hand (and sometimes use an LLM to automate the work)
2. Learn just enough to be able to get an LLM to write code for you (and hope it's correct)

I would recommend option 1 because you need to understand the code to check it is correct, and it will promote your understanding.

But as a guide, here's how I might solve this problem with ChatGPT as an assistant (other tools work in much the same way).


:::{.tip}

**Never paste identifiable or confidential participant data into a chatbot.** Share only column names, a few made-up example rows, or summaries like the ones below, and check your university's guidance on using AI tools with research data.
...
I use these commands to output some information about the data, in a format that's easy to copy into ChatGPT.
...
#### Step 2: Ask ChatGPT to recode it for us

This link shows how I asked ChatGPT to recode the `sweets` data:

<https://chatgpt.com/share/325b1f77-c68c-417a-8e75-76b05a6b9633>
...
#### Step 3: Debug and validate the R code ChatGPT provides
...
This code runs without any errors. **But it is wrong.** Look carefully at the numbers ChatGPT assigned:

- `"I don't like them"` = 0
- `"I like them"` = 1
- `"I'm neutral"` = 2

This scores being neutral *higher* than liking sweets. The responses are ordered (don't like < neutral < like), so the numbers should be too. The mapping should have been:
...
If we hadn't checked, we would have calculated averages and summary scores that looked plausible but were meaningless.

**This is the key lesson: code that runs is not necessarily code that is correct.** R will happily do the wrong thing if you ask it to. You must read and check any code an LLM gives you, just as you would check your own.

It's also worth saying that LLM code *often* doesn't work on first try, and it is dependent on writing a good prompt to the model, and giving it plenty of information about the data and what you want to achieve.
...
- Always check the output (e.g. with `glimpse` or `count`) to make sure it has done what you intended.



Where there are bugs in an LLM's code, you can

- fix these bugs by hand, or
- run the code, report the errors to the LLM and ask it to fix them for you.

The second option has some risks (the code _might_ delete or overwrite your data, although it's unlikely), and is often slower. In this instance it took me quite a while to get ChatGPT to fix the simple errors, and this required understanding the code to some degree anyway. And as we saw above, some mistakes don't produce errors at all: you only find them by reading the code.

As I recommend in the lectures, knowing how to use R well enough to fix the small errors introduced is likely to be more
productive in the long run. Think of LLMs as your assistant rather than your replacement.
...
- Create a summary score called `sleep_problems` which is the sum of these recoded questions (use mutate)
...
sleep %>% 
  select(`My sleep is disturbed by external factors e.g. loud cars, housemates, lights, children...`) %>% 
  unique() %>% paste %>% cat
```


```{r}
# we will use the same mapping for all three questions because they have the same responses
sleep.map <- c("Agree"=2, 
               "Somewhat agree"=1, 
               "Somewhat disagree"=-1, 
               "Disagree"=-2, 
...
sleep.tidy <- sleep %>% 
  # now we recode the three text variables (we only need use mutate once though)
  mutate(
    ...
    sleep_disturbed = recode(`My sleep is disturbed by external factors e.g. loud cars, housemates, lights, children...`, !!!sleep.map)
  )
...
sleep.tidy %>% 
  pivot_longer(c(sleep_study, sleep_electronic, sleep_disturbed)) %>% 
  ggplot(aes(name, value)) + geom_boxplot()
```

And make a summary score combining the three questions. All three items ask whether something *affects* or *disturbs* sleep, so agreeing means more sleep problems. That's why I've called the score `sleep_problems`: higher scores mean *worse* sleep.

```{r}
sleep.tidy.withsummary <- sleep.tidy %>% 
  # and create the summary score
  mutate(sleep_problems = sleep_study + sleep_electronic + sleep_disturbed) 
  
# check the result. it looks ok
sleep.tidy.withsummary %>% glimpse
```
...
sleep.tidy.withsummary %>% 
  ggplot(aes(sleep_problems)) + 
  geom_density()
```
...
- Why is `pivot_longer` useful when we want to make a faceted plot? (Think back to the facets we used in the visualisation session.)
...
- In this worksheet we used `recode` with a mapping variable. What type of values did we start with, and what type did we end up with?
