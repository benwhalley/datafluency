...
add log scale stuff back in
make forward pointer to labs() function qhich comes later... say not to worry yet about labels


This morning I noticed that "craft" might be a useful simile for plotting. If you can continously keep in mind the "customer" rather than the tools, then you're likely to optimise the happiness.

e.g.
don't build someone a chair when they want a table because that's all you know how to make
be prepared to add new tools to your bag to deliver what's required
copy master craftspersons
...
Putting the reader first seems to be the secret to making continuous improvement.
...
- The part which reads `scale_log_10()` is [explained in more detail in this extension exercise](extensions-visualisation.html#logscales)
...
[Extension exercises to supplement this worksheet are available here](visualisation-extensions.html)
