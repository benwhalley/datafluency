...
For example, we could calculate the median `mpg` of all cars in the dataset and overlay that as a horizontal line:
...
***Explanation of the code*** First we calculate the median of `mpg` using summarise. Then we use the `pull(1)` command for the first  time. This selects the first column of data (containing our median) and returns only that. That is, it returns only a number, or a list of numbers, rather than the whole dataframe. Then we added the `geom_hline` function to our existing plot. We set the `yintercept` option to the stored `med_mpg` value, and this defines the height of the line. We added `color="red"` to make the added line distinctive.

***Explanation of the output*** The plot is the same as before, but now has a red line marked at the median of the `mpg` column. The red line is on top of the other plot elements because we added this line at the end of our plotting code.
...
4. Add a horizontal line to the plot, with the `yintercept` set to the median anxiety score.
...
- In the original plot, each marker represents a value *10 times larger* than the previous. So, 1000, 10,000 and 100,000 (the values shown are in [scientific notation](https://www.calculatorsoup.com/calculators/math/scientific-notation-converter.php), so `1e+03` means 1000).
...
***Explanation of the output*** The plot changes, and the distribution is less skewed. We can see that the scale markers are again in scientific notation, and the gaps between points of the scale are not equal: each point on the scale is 10 times larger than the previous one (1000, 10,000, 100,000), stretching out the values across the x axis and reducing the skew.
...
*This is an entirely optional exercise. It is not required for the course assessment and is included only because
...
:::{.tip}
`gganimate` is already installed on the school's RStudio server. If you are using R on your own
computer and see the error `there is no package called 'gganimate'`, install it (and `gifski`,
which it uses to make the animated GIF) by running this once in the **Console**:
...
The text `{frame_time}` inside the title is replaced by the year currently shown in each frame of
the animation.
...
development %>%
  ...
  geom_boxplot() + 
  labs(title = "Life expectancy 1952 vs. 2002", x="Year", y="Life expectancy (years)") +
...
development %>%
  ...
  stat_summary(geom="line", fun.data=median_iqr) + 
  theme_minimal() +
  labs(title = "Life expectancy between 1952 and 2007 (median and IQR)", x="Year", 
...
**Explanation of the output**: We see smoothed lines (one for each country) rather than points. It's pretty hard to distinguish different countries or make sense of anything!
...
In the examples above we have mostly been plotting continuous variables (and adding colours etc),
using histograms, scatter plots and smoothed lines.
...
# simulated RTs: a shifted log-normal distribution (always positive, right-skewed)
set.seed(1234)
expdata <- expand_grid(Condition=c("Low", "Med", "High"), stimuli=1:4, p=1:20) %>%
  mutate(
    Condition = factor(Condition, levels=c("Low", "Med", "High")),
    RT = round(150 + exp(rnorm(n(), log(220) + .1*as.numeric(Condition) - .25*(stimuli==1), .5))),
    stimuli = paste("Stimulus", stimuli))
write_csv(expdata, 'data/expdata.csv')
expdata <- read_csv('data/expdata.csv') %>%
  mutate(Condition = fct_relevel(Condition, "Low", "Med", "High"))
...
```{r, eval=F, echo=T}
expdata <- read_csv('https://benwhalley.github.io/datafluency/data/expdata.csv') %>%
  mutate(Condition = fct_relevel(Condition, "Low", "Med", "High"))
expdata %>% glimpse
```
...
file over the internet, and stores it in the variable called `expdata`. You can click [this link](https://benwhalley.github.io/datafluency/data/expdata.csv) to see the raw data.
The `mutate(Condition = fct_relevel(...))` line sets the order of the `Condition` categories
(otherwise R sorts them alphabetically: High, Low, Med), so plots show Low, Med, High from left to right.
We then use `glimpse` to check the data.


:::{.exercise}

Load the (simulated) dataset from this url: <https://benwhalley.github.io/datafluency/data/expdata.csv> (remember to set the order of the `Condition` categories, as shown above).


-   Recreate the boxplot above
-   Use a facet to recreate the plot you saw at the start of this section, enabling comparisons across `Condition` (on the x axis) and `stimuli` (in separate panels).
...
When you run this in RStudio you will see the message `No summary function supplied, defaulting to mean_se()`.
This is not an error: it just tells you that ggplot is plotting the mean and standard error (`mean_se`) for each group.
...
And you can combine multiple calls to `stat_summary` to get a bar chart with error bars on top:
...
Consider the following two plots. Both present the same data (from a US survey, so incomes are in dollars) on the incomes of men and 
...
However, we should be cautious about point 3: only 26 people in the dataset are blind (10 women
and 16 men), so the means for this group are very uncertain (note the wide error bars). The
conclusion is also not robust: if we compare *medians* rather than means, the pay gap is actually
*smaller* for blind people (see the discussion of skewed incomes below).
...
earnings  %>%
  ...
  facet_grid(~gender) + 
  stat_summary() +
  labs(y = "Income ($1000)", x="Education")
```
`r hide('Show hint')`
...
When we add a log scale, the gender pay gaps appear more similar across the different education
categories.
