...
fuel %>% ggplot(aes(power, mpg)) + geom_point()

happy %>% filter(year>2008) %>% summarise(mean(happiness)) %>% .[1,1]

fuel %>% filter(cyl==6) %>% summarise(mean(mpg)) %>% .[1,1]

earnings %>% ggplot(aes(gender, hours)) + geom_boxplot()
fuel %>%
  group_by(cyl) %>%
...
- Make sure you are familiar with all the  [plotting](02-plotting.html)
