```{r, cache=T}
# a simulation to show that p values are normally distributed
sim.result <- expand.grid(sim.run=1:10e4, g=0:1, p=1:20) %>% mutate(y=rnorm(n())) %>%
  group_by(sim.run) %>%
  do(t.test(y~g, data=.) %>% broom::tidy(.))

sim.result %>%
  ggplot(aes(p.value)) + geom_histogram(bins=50)
```
...
anova(m)
m.emm <- emmeans::emmeans(m, "Days")
m.emm

m.emm.df <- m.emm %>%
  broom::tidy(conf.int = TRUE) # this line changed
