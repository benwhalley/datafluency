...
Work in pairs to brainstorm what factors incfluence a phenomena that interests you in applied psychology. 
It might be related to something like clinical outcomes, or perhaps workplace stress.
...
Load the data from <https://benwhalley.github.io/rmip/data/studyhabitsandgrades.csv> and reproduce the colored plot from above.
...
studyhabits %>% 
  ...
  geom_abline(slope=coef(m1)[3], intercept=coef(m1)[1]+coef(m1)[2], color=gg_color_hue(2)[2]) +
  coord_cartesian(xlim=c(0,40)) + guides(color=F)
...
As [we did for single-predictor regression](04-regression.html#regression-in-r) we can use `lm` to get numbers to describe the
...
## The 'probability of your model' {#lm-bf}
...
something like `[1] work_hours * female : 8.327853e+12 ±0%`. This is the Bayes
...
so BF01 < 1/3 means substantial evidence for a difference, and BF01 > 3 means
