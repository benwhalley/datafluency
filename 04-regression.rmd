...
## Thinking about causes {#regression2}
...
digraph {
    {node [shape=box];}
    Sunshine -> Rainbows -> "Rainbows later" -> "Sunshine even later" -> "..."
  }
', height=400)
```
...
grViz('
digraph {
    "Low pressure weather system" -> Sunshine
     "Low pressure weather system" -> Rainbows
   }
', height=200)
```
...
-   A common cause that is not in the model

---

### Task {#causes-task-make-a-diagram}
...
#### Task {#diagram-task-moderation-followup .exercise}
...
1.  Draw a causal diagram based ONLY on the data in the plot

2.  Redraw the diagram adding at least one extra variable to your diagram, and converting any
    double-headed arrows ($\leftrightarrow$) to single-headed arrows.

3.  Discuss in your group what you think is happening here. Have you come across this idea before?

[See the explanation](#confounding-explanation)
...
'hungover' was recorded as categorical (e.g. not at all/a little/very) when we might use Anova, and
if 'hungover' was recorded as continuous score (e.g. from 1 to 100) we might use regression.
...
We cover multiple regression, and basic statistical testing for individual coefficients and for
models using frequentist and bayesian methods (regular _p_ values).
